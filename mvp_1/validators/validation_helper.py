...
def check_username_rules(v: str):
    if v.startswith(("_", ".")) or v.endswith((".", "_")):
        raise UsernameValidationException("Username cannot start or end with '_' or '.'")

    if "version_0_1." in v or "__" in v:
        raise UsernameValidationException("Username cannot contain consecutive periods or underscores")
    
    if v in prohibited_usernames:
        raise UsernameValidationException("Username is prohibited")
    return v
...
def check_password_rules(v: SecretStr):
    if " " in v.get_secret_value():
        raise PasswordValidationException("Password cannot contain spaces")
    return v
...
def process_mail(value: str):
    ...
    try:
        ...
        raise EmailValidationException("Invalid email format") from e
