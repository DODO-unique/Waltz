...
class Orchestra:
    ...
    async def local_authenticate(self, payload: LocalAuthPayload):
        ...
        if not result.value:
            ...
            raise TypeMismatchException(str(result))
    ...
    def _oauth_registration_from_verified_claims(self, verified_claims: dict[str, Any]) -> OAuthRegistration:
        ...
        if subject is None:
            raise OIDCTokenInvalidException("OIDC id_token missing required 'sub' claim")
        ...
        if updated_at is None:
            raise OIDCTokenInvalidException("OIDC id_token must contain 'updated_at' or 'iat' claim")
    ...
    def _parse_datetime(self, value: Any) -> datetime | None:
        ...
        raise OIDCTokenInvalidException("Cannot parse datetime from OIDC claim")
    ...
    async def _oidc_oauth_registration(self, payload: JWKVerificationRequest) -> UUID:
        ...
        if not isinstance(oauth_payload.id, str):
            raise TypeMismatchException("OAuth payload is not sting.")
