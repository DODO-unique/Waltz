...
from validation_helper import Payload, ProviderName
...
class Listeners:
    ...
    def decorator(self, record_enum: TicketEnum):
        def actual_decorator(
            user_func: Callable[[Payload], Awaitable[Payload]]
        ) -> Callable[[Payload], Awaitable[Payload]]:

            async def wrapper(payload: Payload) -> Payload:
                return await user_func(payload)

            package = DatabaseRegistry(
                operation=record_enum,
                operator=wrapper,
            )
